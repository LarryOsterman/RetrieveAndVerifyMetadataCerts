...
public:
    ...
    std::vector<uint8_t> GetRawPublicKey()
    {
        uint8_t* rawPublicKeyStart = _certificateContext.get()->pCertInfo->SubjectPublicKeyInfo.PublicKey.pbData;
        return std::vector<uint8_t>(rawPublicKeyStart, rawPublicKeyStart + _certificateContext.get()->pCertInfo->SubjectPublicKeyInfo.PublicKey.cbData);
    }

    std::vector<uint8_t> ExportPublicKey()
    {

        DWORD oidSequenceSize;
        wil::unique_hlocal oidSequence;
        {
            ...
            THROW_IF_WIN32_BOOL_FALSE(CryptEncodeObjectEx(X509_ASN_ENCODING, X509_SEQUENCE_OF_ANY, oidSequenceOfAny, CRYPT_ENCODE_ALLOC_FLAG, nullptr, oidSequence.addressof(), &oidSequenceSize));
        }
        ...
        DWORD keySize;
        wil::unique_hlocal keyBitString;
        THROW_IF_WIN32_BOOL_FALSE(CryptEncodeObjectEx(X509_ASN_ENCODING, X509_BITS, &bitString, CRYPT_ENCODE_ALLOC_FLAG, nullptr, keyBitString.addressof(), &keySize));
        ...
        keySequenceOfAny->rgValue[0].cbData = oidSequenceSize;
        keySequenceOfAny->rgValue[0].pbData = reinterpret_cast<BYTE*>(oidSequence.get());
        keySequenceOfAny->rgValue[1].cbData = keySize;
        keySequenceOfAny->rgValue[1].pbData = reinterpret_cast<BYTE*>(keyBitString.get());

        DWORD publicKeySize;
        wil::unique_hlocal keySequence;
        THROW_IF_WIN32_BOOL_FALSE(CryptEncodeObjectEx(X509_ASN_ENCODING, X509_SEQUENCE_OF_ANY, keySequenceOfAny, CRYPT_ENCODE_ALLOC_FLAG, nullptr, &keySequence, &publicKeySize));
        return std::vector<uint8_t>(reinterpret_cast<uint8_t*>(keySequence.get()), reinterpret_cast<uint8_t*>(keySequence.get())+publicKeySize);
    }
